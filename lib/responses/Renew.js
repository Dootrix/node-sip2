...
class RenewResponse extends ResponseMessage {
    parse(message) {
        ...
        let feeType = this.parseVariable('BT', this.message.substring(24));
        data.feeType = FeeType.parse(feeType);
        let securityInhibit = this.parseVariable('CI', this.message.substring(24));
        data.securityInhibit = this.charToBool(securityInhibit);
        let currency = this.parseVariable('BH', this.message.substring(24));
        ...
        // Optional fixed length
        let mediaType = this.parseVariable('CK', this.message.substring(24));
