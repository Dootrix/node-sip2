...
class ItemInformationResponse extends ResponseMessage {
  ...
  parse(message) {
    ...
    const dueDate = this.parseVariableWithoutDelimeter('AH', this.message.substring(26));
    if (dueDate) data.dueDate = this.parseDateTime(dueDate);
    ...
    data.currencyType = this.parseVariable('BH', this.message.substring(26));
    data.feeAmount = this.parseVariable('BV', this.message.substring(26));
